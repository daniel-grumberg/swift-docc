...
class ExtendedTypesFormatTransformationTests: XCTestCase {
    ...
    func testExtendedTypesFormatStructure() throws {
        ...
        let addedMemberSymbolsTypeATwo = graph.symbols.values.filter({ symbol in
            symbol.kind.identifier == .property && symbol.pathComponents[symbol.pathComponents.count - 2] == "ATwo"
        })
    ...
    func testExtendedNestedTypeHierarchySynthesis() throws {
        let contents =
            twoExtensionBlockSymbolsExtendingSameType(
                extendedModule: "A",
                extendedType: "A",
                withExtensionMembers: false,
                pathPrefix: ["Unextended", "Extended", "UnextendedInner"]
            )
        ...
        let extendedTypeUnextendedDotExtended = try XCTUnwrap(
            graph.symbols.values.first(where: { symbol in symbol.kind.identifier == .extendedStructure && symbol.title == "Unextended.Extended" })
        )

        let extendedTypeUnextendedDotExtendedDotUnextendedInner = try XCTUnwrap(
            graph.symbols.values.first(where: { symbol in symbol.kind.identifier == .unknownExtendedType && symbol.title == "Unextended.Extended.UnextendedInner" })
        )

        let extendedTypeUnextendedDotExtendedDotUnextendedInnerDotA = try XCTUnwrap(
            graph.symbols.values.first(where: { symbol in symbol.kind.identifier == .extendedStructure && symbol.title == "Unextended.Extended.UnextendedInner.A" })
        )
    ...
    private func twoExtensionBlockSymbolsExtendingSameType(
        ...
        return SymbolGraphContents(
                ...
                + (withExtensionMembers
                    ? [
                        SymbolKit.SymbolGraph.Symbol(
                            ...
                            mixins: [
                                SymbolGraph.Symbol.Swift.Extension.mixinKey: SymbolGraph.Symbol.Swift.Extension(
            ...
            relationships: [
                .init(
                    source: "s:e:s:\(extendedModule)\(pathPrefix.joined())\(extendedType)two",
                    target: "s:\(extendedModule)\(pathPrefix.joined())\(extendedType)",
                    kind: .extensionTo,
                    targetFallback: "\(extendedModule).\(titlePrefix)\(extendedType)"
                ),
                .init(
                    source: "s:e:s:\(extendedModule)\(pathPrefix.joined())\(extendedType)one",
                    target: "s:\(extendedModule)\(pathPrefix.joined())\(extendedType)",
                    kind: .extensionTo,
                    targetFallback: "\(extendedModule).\(titlePrefix)\(extendedType)"
                ),
                ...
                + (withExtensionMembers
                    ? [
                        .init(
                            source: "s:\(extendedModule)\(pathPrefix.joined())\(extendedType)two",
                            target: "s:e:s:\(extendedModule)\(pathPrefix.joined())\(extendedType)two",
                            kind: .memberOf,
                            targetFallback: "\(extendedModule).\(titlePrefix)\(extendedType)"
                        ),
                        .init(
                            source: "s:\(extendedModule)\(pathPrefix.joined())\(extendedType)one",
                            target: "s:e:s:\(extendedModule)\(pathPrefix.joined())\(extendedType)one",
                            kind: .memberOf,
                            targetFallback: "\(extendedModule).\(titlePrefix)\(extendedType)"
                        ),
                    ] : [])
    ...
    private func allPermutations(
        of symbols: [SymbolGraph.Symbol],
        and relationships: [SymbolGraph.Relationship]
