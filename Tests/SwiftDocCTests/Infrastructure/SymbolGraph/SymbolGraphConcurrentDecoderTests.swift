...
class SymbolGraphConcurrentDecoderTests: XCTestCase {
    ...
    private let lines = SymbolGraph.LineList(
        Array(repeating: SymbolGraph.LineList.Line(text: "Apple Banana Orange Pear Mango Kiwi Grapefruit Melon Watermelon", range: nil), count: 20)
    )
    private func makeSymbol(index: Int) -> SymbolGraph.Symbol {
        return SymbolGraph.Symbol(
            ...
            names: .init(
                title: "Symbol \(index)",
                navigator: nil,
                subHeading: [SymbolGraph.Symbol.DeclarationFragments.Fragment(kind: .identifier, spelling: "Symbol \(index)", preciseIdentifier: UUID().uuidString)],
                prose: "Symbol \(index)"
            ),
    ...
    func testEncodingDecodingConcurrently() throws {
        ...
        let topLevelCurationSGFURL = Bundle.module.url(
            forResource: "TopLevelCuration.symbols",
            withExtension: "json",
