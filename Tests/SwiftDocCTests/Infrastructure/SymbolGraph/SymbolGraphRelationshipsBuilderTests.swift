...
class SymbolGraphRelationshipsBuilderTests: XCTestCase {
    ...
    private func createSymbols(
        ...
        let sourceSymbol = SymbolGraph.Symbol(
            identifier: sourceIdentifier,
            names: SymbolGraph.Symbol.Names(title: "A", navigator: nil, subHeading: nil, prose: nil),
            pathComponents: ["MyKit", "A"],
            docComment: nil,
            accessLevel: .init(rawValue: "public"),
            kind: sourceType,
            mixins: [:]
        )
        let targetSymbol = SymbolGraph.Symbol(
            identifier: targetIdentifier,
            names: SymbolGraph.Symbol.Names(title: "B", navigator: nil, subHeading: nil, prose: nil),
            pathComponents: ["MyKit", "B"],
            docComment: nil,
            accessLevel: .init(rawValue: "public"),
            kind: targetType,
            mixins: [:]
    ...
    func testImplementsRelationship() throws {
        ...
        let edge = createSymbols(
            documentationCache: &documentationCache,
            bundle: bundle,
            sourceType: .init(parsedIdentifier: .class, displayName: "Class"),
            targetType: .init(parsedIdentifier: .protocol, displayName: "Protocol")
        )

        // Adding the relationship
        SymbolGraphRelationshipsBuilder.addImplementationRelationship(
            edge: edge,
            selector: swiftSelector,
            in: bundle,
            context: context,
    ...
    func testConformsRelationship() throws {
        ...
        guard
            let conformsTo = (documentationCache["A"]!.semantic as! Symbol).relationships.groups
                .first(where: { group -> Bool in
        ...
        guard
            let conforming = (documentationCache["B"]!.semantic as! Symbol).relationships.groups
                .first(where: { group -> Bool in
    ...
    func testInheritanceRelationship() throws {
        ...
        guard
            let inherits = (documentationCache["A"]!.semantic as! Symbol).relationships.groups
                .first(where: { group -> Bool in
        ...
        guard
            let inherited = (documentationCache["B"]!.semantic as! Symbol).relationships.groups
                .first(where: { group -> Bool in
    ...
    func testRequirementRelationship() throws {
        ...
        let edge = createSymbols(
            documentationCache: &documentationCache,
            bundle: bundle,
            sourceType: .init(parsedIdentifier: .method, displayName: "Method"),
            targetType: .init(parsedIdentifier: .protocol, displayName: "Protocol")
        )
