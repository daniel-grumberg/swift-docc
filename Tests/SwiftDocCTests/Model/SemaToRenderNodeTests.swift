...
class SemaToRenderNodeTests: XCTestCase {
    func testCompileTutorial() throws {
        ...
        XCTAssertEqual(
            ...
            ResolvedTopicReference(bundleIdentifier: "org.swift.docc.example", path: "/tutorials/Test-Bundle/TestTutorial", sourceLanguage: .swift)
        )
        ...
        if let testTutorialFirstSectionReference = renderNode.references["doc://org.swift.docc.example/tutorials/Test-Bundle/TestTutorial#Create-a-New-AR-Project-%F0%9F%92%BB"]
        ...
        if let testTutorial2FirstSectionReference = renderNode.references["doc://org.swift.docc.example/tutorials/Test-Bundle/TestTutorial2#Create-a-New-AR-Project"]
            as? TopicRenderReference
        {
        ...
        if let testTutorialSecondSectionReference = renderNode.references["doc://org.swift.docc.example/tutorials/Test-Bundle/TestTutorial#Initiate-ARKit-Plane-Detection"]
            as? TopicRenderReference
        {
        ...
        XCTAssertEqual(
            downloadReference.checksum,
            "2521bb27db3f8b72f8f2bb9e3a33698b9c5c72a5d7862f5b209794099e1cf0acaab7d8a47760b001cb508b5c4f3d7cf7f8ce1c32679b3fde223e63b5a1e7e509"
        )
            ...
            .contentSection.first
        {
            ...
            XCTAssertFalse(
                contentAndMedia.content.firstParagraph.contains(
                    .reference(identifier: .init("doc:ThisWillNeverResolve"), isActive: true, overridingTitle: nil, overridingTitleInlineContent: nil)
                )
            )
        } else {
    ...
    private func assertCompileOverviewWithNoVolumes(bundle: DocumentationBundle, context: DocumentationContext, expectedProblemsCount: Int = 0) throws {
        ...
        renderNode.references.compactMap { $0.value as? TopicRenderReference }
            .forEach {
    ...
    func testCompileSymbol() throws {
        ...
        guard
            let languageTrait = renderNode.variants?.first?.traits
                .first(where: { trait -> Bool in
        ...
        XCTAssertEqual(
            renderNode.seeAlsoSections[0].abstract?.map { RenderBlockContent.paragraph(.init(inlineContent: [$0])) }.paragraphText.joined(),
        ...
        XCTAssertTrue(
            Array(renderNode.seeAlsoSections[0].identifiers.prefix(3))
                .allSatisfy({ identifier -> Bool in
                    return identifier == "doc://org.swift.docc.example/documentation/MyKit/MyClass"
                })
        )
    ...
    func testRenderConstraints() throws {
        ...
        XCTAssertEqual(
            conf.constraints
                ...
                .joined(),
            "Label is Text, Observer inherits NSObject, and S conforms to StringProtocol."
        )
        ...
        XCTAssertEqual(
            functionReference.conformance?.constraints
                .map { el -> String in
                    switch el {
                    case .text(let text): return text
                    case .codeVoice(let text): return text
    ...
    func testRenderConditionalConstraintsOnConformingType() throws {
        ...
        XCTAssertEqual(
            myClassRelationship.conformance?.constraints
                .map { el -> String in
                    switch el {
                    case .text(let text): return text
                    case .codeVoice(let text): return text
    ...
    func testRenderConditionalConstraintsOnProtocol() throws {
        ...
        XCTAssertEqual(
            myProtocolRelationship.conformance?.constraints
                .map { el -> String in
                    switch el {
                    case .text(let text): return text
                    case .codeVoice(let text): return text
            ...
            let (_, bundle, context) = try testBundleAndContext(
                ...
                configureBundle: { url in
                    ...
                    for testData in allPlatformsNames {
                        ...
                        availability.availability.append(
                            .init(
                                domain: SymbolGraph.Symbol.Availability.Domain(rawValue: testData.platformName),
    ...
    func testMediaReferencesWithSpaces() throws {
        ...
        let node = try context.entity(
            with: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/tutorials/Test-Bundle/TutorialMediaWithSpaces", sourceLanguage: .swift)
        )
    ...
    func testUnexpectedDirectivesAreDropped() throws {
        ...
        let (bundle, context) = try testBundleAndContext(named: "TestBundle")
        var contentTranslator = RenderContentCompiler(
            context: context,
            bundle: bundle,
            identifier: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/TestTutorial", sourceLanguage: .swift)
        )
    ...
    func testArticleRoleHeadings() throws {
        try assertRoleHeadingForArticleInTestBundle(
    ...
    */
    ...
    func testWarnForExternalLinksInTopicTaskGroups() throws {
        ...
        XCTAssertNotNil(
            context.problems.first(where: { problem -> Bool in
    ...
    func testRendersBetaViolators() throws {
        ...
        do {
            ...
            renderReferenceSymbol.availability?.availability
                .append(
                    SymbolGraph.Symbol.Availability.AvailabilityItem(
                        domain: SymbolGraph.Symbol.Availability.Domain(rawValue: "ImaginaryOS"),
        ...
        do {
            ...
            (node.semantic as? Symbol)?.availability = SymbolGraph.Symbol.Availability(availability: [
                SymbolGraph.Symbol.Availability.AvailabilityItem(
                    ...
                    introducedVersion: nil,
                    deprecatedVersion: .init(major: 13, minor: 0, patch: 0),
                    obsoletedVersion: nil,
                    message: nil,
                    renamed: nil,
                    isUnconditionallyDeprecated: false,
        ...
        do {
            ...
            (node.semantic as? Symbol)?.availability = SymbolGraph.Symbol.Availability(availability: [
                ...
                SymbolGraph.Symbol.Availability.AvailabilityItem(
                    ...
                    introducedVersion: nil,
                    deprecatedVersion: .init(major: 10, minor: 15, patch: 0),
                    obsoletedVersion: nil,
                    message: nil,
                    renamed: nil,
                    isUnconditionallyDeprecated: false,
    ...
    func testRendersDeprecatedViolatorForUnconditionallyDeprecatedReference() throws {
        ...
        do {
            ...
            (node.semantic as? Symbol)?.availability = SymbolGraph.Symbol.Availability(availability: [
                ...
                SymbolGraph.Symbol.Availability.AvailabilityItem(
                    domain: .init(rawValue: "macOS"),
                    introducedVersion: .init(major: 11, minor: 0, patch: 0),
                    deprecatedVersion: nil,
                    obsoletedVersion: nil,
                    message: nil,
                    renamed: nil,
    ...
    func testDefaultImplementations() throws {
        ...
        do {
            let node = try context.entity(
                with: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/documentation/SideKit/SideProtocol/func()-6ijsi", sourceLanguage: .swift)
            )
            let symbol = node.semantic as! Symbol
            var translator = RenderNodeTranslator(context: context, bundle: bundle, identifier: node.reference)
            let renderNode = translator.visit(symbol) as! RenderNode
    ...
    func testRemovingTrailingNewLinesInDeclaration() throws {
        ...
        let node = try context.entity(
            with: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/documentation/MyKit/globalFunction(_:considering:)", sourceLanguage: .swift)
        )
        let symbol = node.semantic as! Symbol
    ...
    func testDocInheritanceInsideModule() throws {
        let sgURL = Bundle.module.url(
            forResource: "TestBundle.docc/sidekit.symbols",
            withExtension: "json",
            subdirectory: "Test Bundles"
        )!
        ...
        for testData in testData {
            ...
            let (_, bundle, context) = try testBundleAndContext(
                ...
                configureBundle: { url in
                    ...
                    graph.symbols["s:7SideKit0A5::SYNTHESIZED::inheritedFF"]?.docComment = try JSONDecoder()
                        .decode(SymbolGraph.LineList.self, from: testData.docCommentJSON.data(using: .utf8)!)
    ...
    func testEnabledDocInheritance() throws {
        ...
        XCTAssertFalse(
            context.diagnosticEngine.problems.contains(where: { problem in
    ...
    func testInheritedAPIGroupsInCollidedParents() throws {
        ...
        let inheritedSymbolReference = ResolvedTopicReference(
            bundleIdentifier: bundle.identifier,
            path: "/documentation/Minimal_docs/A/B-swift.struct/!=(_:_:)",
            sourceLanguage: .swift
        )
        ...
        let equatableImplementationsReference = ResolvedTopicReference(
            bundleIdentifier: bundle.identifier,
            path: "/documentation/Minimal_docs/A/B-swift.struct/Equatable-Implementations",
            sourceLanguage: .swift
        )
    ...
    func testThematicBreak() throws {
        ...
        var contentTranslator = RenderContentCompiler(
            context: context,
            bundle: bundle,
            identifier: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/TestThematicBreak", sourceLanguage: .swift)
        )
