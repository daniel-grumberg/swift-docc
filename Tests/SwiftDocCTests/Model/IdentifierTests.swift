...
class IdentifierTests: XCTestCase {
    ...
    func testReusingReferences() {
        ...
        XCTAssertEqual(
            ...
            1,
            "Should have an cached one reference because a reference with this bundle identifier was created"
        )
        ...
        XCTAssertEqual(
            ResolvedTopicReference._numberOfCachedReferences(bundleID: bundleID),
            1,
            "Should still only have one cached reference because the same reference was created repeatedly"
        )
        ...
        XCTAssertEqual(
            ResolvedTopicReference._numberOfCachedReferences(bundleID: bundleID),
            2,
            "Should have cached another reference because two different references with this bundle identifier has been created"
        )
    ...
    func testReferencesAreNotCachedByDefault() {
        ...
        XCTAssertNil(
            ResolvedTopicReference._numberOfCachedReferences(bundleID: bundleID),
            "After creating a reference in this bundle, references still shouldn't exist because caching is not enabled by default"
        )
    }
