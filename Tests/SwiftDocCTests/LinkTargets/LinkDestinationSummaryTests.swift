...
class ExternalLinkableTests: XCTestCase {
    ...
    let bundleFolderHierarchy = Folder(
        name: "unit-test.docc",
        content: [
    ...
    func testSymbolSummaries() throws {
        ...
        do {
            ...
            XCTAssertEqual(
                summary.taskGroups?.map { $0.title },
                [
    ...
    func testTopicImageReferences() throws {
        ...
        do {
            ...
            do {
                ...
                XCTAssertEqual(
                    Set(imageReference.asset.variants.keys),
                    [
            ...
            summary.references = summary.references?
                .compactMap { (original: RenderReference) -> RenderReference? in
                    ...
                    imageRef.asset.metadata = .init(
                        uniqueKeysWithValues: imageRef.asset.metadata.map { key, value in
    ...
    func testOverloadSymbolsWithManualCuration() throws {
        ...
        let node = try context.entity(
            with: ResolvedTopicReference(bundleIdentifier: bundle.identifier, path: "/documentation/MyModule/MyClass/myFunc()-9sdsh", sourceLanguage: .swift)
        )
        let renderNode = try converter.convert(node)
