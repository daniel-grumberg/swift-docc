...
class NavigatorIndexingTests: XCTestCase {
    ...
    func testNavigationTreeDumpAndRead() throws {
        ...
        let treeWithPresentationIdentifier = try NavigatorTree.read(
            from: indexURL,
        ...
        func addAttributes(node: NavigatorTree.Node) {
    ...
    func disabled_testNavigationTreeLargeDumpAndReadAsync() throws {
        ...
        try! readTreePresentationIdentifier.read(from: indexURL, timeout: 0.25, queue: DispatchQueue.main, presentationIdentifier: "com.example.test") {
            (nodes, completed, error) in
            XCTAssertNil(error)
                    ...
                    content: makeSymbolGraph(
                        ...
                        symbols: [
                            ...
                            .init(
                                ...
                                names: .init(
                                    title: "OtherSymbol",
                                    navigator: [.init(kind: .identifier, spelling: "OtherSymbol", preciseIdentifier: nil)],
                                    subHeading: nil,
                                    prose: nil
                                ),
    ...
    func testNavigatorWithDifferentSwiftAndObjectiveCHierarchies() throws {
        ...
        let fromMemoryBuilder = NavigatorIndex.Builder(
            outputURL: try createTemporaryDirectory(),
            bundleIdentifier: bundle.identifier,
            sortRootChildrenByName: true,
            groupByLanguage: true
        )
        let fromDecodedBuilder = NavigatorIndex.Builder(
            outputURL: try createTemporaryDirectory(),
            bundleIdentifier: bundle.identifier,
            sortRootChildrenByName: true,
            groupByLanguage: true
    ...
    func testPlatformVersion() {
        ...
        let bitVersion: UInt32 = 0b00000000_00001101_00000011_00000000
    ...
    func testAvailabilityIndexInterfaceLanguageBackwardsCompatibility() throws {
        ...
        let availabilityIndex = try PropertyListDecoder()
            .decode(
...
private func compare(lhs: Node, rhs: Node) -> Bool {
...
private func search(node: Node, matching predicate: (Node) -> Bool) -> Node? {
...
private func validateTree(node: NavigatorTree.Node, validator: (NavigatorTree.Node) -> Bool) -> Bool {
    if validator(node) == false { return false }
    for child in node.children where validateTree(node: child, validator: validator) == false {
        return false
    }
    return true
}

private func assertUniqueIDs(node: NavigatorTree.Node, message: String = "The tree has duplicated IDs.", file: StaticString = #file, line: UInt = #line) {
...
private func testTree(named name: String) throws -> String {
