...
class OutOfProcessReferenceResolverTests: XCTestCase {
    ...
    func testMetadataMessageWithVariants() throws {
        ...
        do {
            ...
            case .resolvedInformation(let decodedInformation):
                ...
                for (decodedFragment, testFragment) in zip(
                    decodedInformation.declarationFragments?.declarationFragments ?? [],
                    testMetadata.declarationFragments?.declarationFragments ?? []
                ) {
                ...
                for (decodedFragment, testFragment) in zip(
                    decodedVariant.declarationFragments??.declarationFragments ?? [],
                    testVariant.declarationFragments??.declarationFragments ?? []
                ) {
                    XCTAssertEqual(decodedFragment, testFragment)
                }
    ...
    func assertSymbolBetaStatus(
        platforms: [OutOfProcessReferenceResolver.ResolvedInformation.PlatformAvailability],
        expectedStatus isBeta: Bool,
