...
extension NavigatorIndex {
    ...
    open class Builder {
        ...
        public init(
            renderNodeProvider: RenderNodeProvider? = nil,
            outputURL: URL,
            bundleIdentifier: String,
            sortRootChildrenByName: Bool = false,
            groupByLanguage: Bool = false,
            writePathsOnDisk: Bool = true,
            usePageTitle: Bool = false
        ) {
        ...
        public func index(renderNode: RenderNode) throws {
            ...
            guard
                let objCVariantTrait = renderNode.variants?.flatMap(\.traits)
                    .first(where: { trait in
                        switch trait {
            ...
            if let alreadyDefinedEnvironment = navigatorIndex.environment {
                ...
                do {
                    environment = try LMDB.Environment(
                        ...
                        flags: [.noLock],
                        maxDBs: 4,
                        mapSize: 100 * 1024 * 1024  // mapSize = 100MB
                    )
...
extension LMDB.Database {
    ...
    */
    func put(records: [NavigatorIndex.Builder.Record], flags: WriteFlags = []) throws {
        try LMDB.Transaction(environment: environment)
            .run { database in
