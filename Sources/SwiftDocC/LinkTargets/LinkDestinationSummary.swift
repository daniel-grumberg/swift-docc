...
public extension DocumentationNode {
    ...
    func externallyLinkableElementSummaries(
        ...
        let landmarkSummaries = ((semantic as? Tutorial)?.landmarks ?? (semantic as? TutorialArticle)?.landmarks ?? [])
            .compactMap {
        ...
        if includeTaskGroups {
            ...
            default:
                ...
                for variant in renderNode.topicSectionsVariants.variants {
                    taskGroupVariants[variant.traits] = renderNode.topicSectionsVariants.value(for: variant.traits)
                        .map { group in .init(title: group.title, identifiers: group.identifiers) }
                }
            }
        } else {
            taskGroups = nil
...
extension LinkDestinationSummary {
    ...
    init(
        ...
        guard let symbol = documentationNode.semantic as? Symbol,
            let summaryTrait = documentationNode.availableVariantTraits.first(where: { $0.interfaceLanguage == documentationNode.sourceLanguage.id })
        else {
...
private extension [[PlatformName?]: SymbolGraph.Symbol.DeclarationFragments] {
    ...
    func renderDeclarationTokens() -> [DeclarationRenderSection.Token]? {
        return mainRenderFragments()?.declarationFragments
            .map {
...
extension LinkDestinationSummary {
    ...
    init?(landmark: Landmark, relativeParentPresentationURL: URL, page: DocumentationNode, platforms: [PlatformAvailability]?, compiler: inout RenderContentCompiler) {
        ...
        } else if let paragraph = landmark.markup.children.lazy.compactMap({ $0 as? Paragraph }).first,
            case RenderBlockContent.paragraph(let p)? = compiler.visitParagraph(paragraph).first
        {
...
extension LinkDestinationSummary {
    ...
    public init(from decoder: Decoder) throws {
        ...
        availableLanguages = try Set(
            availableLanguageIDs.map { languageID in
                guard let foundLanguage = SourceLanguage.knownLanguages.first(where: { $0.id == languageID }) else {
        ...
        references = try container.decodeIfPresent([CodableRenderReference].self, forKey: .references)
            .map { decodedReferences in
