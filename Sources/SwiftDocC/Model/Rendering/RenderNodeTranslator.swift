...
public struct RenderNodeTranslator: SemanticVisitor {
    ...
    public mutating func visitStep(_ step: Step) -> RenderTree? {
        ...
        let previewReference = step.code?.preview
            .flatMap {
    ...
    public mutating func visitTutorial(_ tutorial: Tutorial) -> RenderTree? {
        ...
        tutorialSections.tasks = tutorialSections.tasks.enumerated()
    ...
    private mutating func callToAction(with callToActionImage: ImageMedia?, volume: ResolvedTopicReference) -> CallToActionSection? {
        ...
        if let nextTopicIndex, nextTopicIndex < surroundingTopics.count {
            ...
            return createCallToAction(
                ...
                abstract: inlineAbstractContentInTopic(nextTopic),
                image: image
            )
        }
    ...
    private mutating func createCallToAction(
    ...
    public mutating func visitMultipleChoice(_ multipleChoice: MultipleChoice) -> RenderTree? {
        ...
        return TutorialAssessmentsRenderSection.Assessment(
            ...
            content: content,
            choices: multipleChoice.choices.map { visitChoice($0) } as! [TutorialAssessmentsRenderSection.Assessment.Choice]
        )
    }
    ...
    public mutating func visitArticle(_ article: Article) -> RenderTree? {
        ...
        if let availability = article.metadata?.availability, !availability.isEmpty {
            let renderAvailability =
                availability.compactMap({
                    let currentPlatform = PlatformName(metadataPlatform: $0.platform)
                        .flatMap { name in
    ...
    private mutating func renderGroups(
        ...
        return topics.taskGroups.compactMap { group in
            let supportedLanguages = group.directives[SupportedLanguage.directiveName]?
                .compactMap {
    ...
    public mutating func visitSymbol(_ symbol: Symbol) -> RenderTree? {
        ...
        if let crossImportOverlayModule = symbol.crossImportOverlayModule {
            node.metadata.modulesVariants = VariantCollection(defaultValue: [
                RenderMetadata.Module(name: crossImportOverlayModule.declaringModule, relatedModules: crossImportOverlayModule.bystanderModules)
            ])
        ...
        node.metadata.platformsVariants =
            VariantCollection<[AvailabilityRenderItem]?>(from: symbol.availabilityVariants) { _, availability in
                availability.availability
                    .compactMap { availability -> AvailabilityRenderItem? in
                        ...
                        guard let name = availability.domain.map({ PlatformName(operatingSystemName: $0.rawValue) }),
                            let currentPlatform = context.externalMetadata.currentPlatforms?[name.displayName]
                        else {
            ...
            VariantCollection<[TaskGroupRenderSection]>(
                ...
                for group in groups {
                    ...
                    if let duplicateSectionIndex = sections.firstIndex(where: { $0.title == group.title }) {
                        ...
                        sections[duplicateSectionIndex] = TaskGroupRenderSection(
                            title: originalSection.title,
                            abstract: originalSection.abstract,
                            discussion: originalSection.discussion,
                            identifiers: combinedReferences
                        )
                    } else {
        ...
        node.deprecationSummaryVariants =
            VariantCollection<[RenderBlockContent]?>(
    ...
    mutating func createAndRegisterRenderReference(
        ...
        guard
            let resolvedAssets = renderContext?.store.content(forAssetNamed: media.path, bundleIdentifier: identifier.bundleIdentifier)
    ...
    mutating func defaultAvailability(for bundle: DocumentationBundle, moduleName: String, currentPlatforms: [String: PlatformVersion]?) -> [AvailabilityRenderItem]? {
        ...
        guard let bundleDefaultAvailability = bundle.info.defaultAvailability,
            let moduleAvailability = bundleDefaultAvailability.modules[moduleName]
        else {
            return nil
        }
...
private typealias BundleModuleIdentifier = String
