...
struct RenderContentCompiler: MarkupVisitor {
        ...
        return [
            RenderBlockContent.heading(
                .init(
                    ...
                    anchor: urlReadableFragment(heading.plainText).addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed)
                )
            )
        ]
    }
    ...
    mutating func visitParagraph(_ paragraph: Paragraph) -> [RenderContent] {
        return [
            RenderBlockContent.paragraph(
                .init(inlineContent: paragraph.children.reduce(into: [], { result, child in result.append(contentsOf: visit(child)) }) as! [RenderInlineContent])
            )
        ]
    }
    ...
    mutating func visitEmphasis(_ emphasis: Emphasis) -> [RenderContent] {
        return [
            RenderInlineContent.emphasis(inlineContent: emphasis.children.reduce(into: [], { result, child in result.append(contentsOf: visit(child)) }) as! [RenderInlineContent])
        ]
    }
    ...
    mutating func visitTable(_ table: Table) -> [RenderContent] {
        ...
        return [
            RenderBlockContent.table(
                .init(header: .row, rawAlignments: alignments, rows: [RenderBlockContent.TableRow(cells: headerCells)] + rows, extendedData: extendedData, metadata: nil)
            )
        ]
    }
    ...
    mutating func visitStrikethrough(_ strikethrough: Strikethrough) -> [RenderContent] {
        return [
            RenderInlineContent.strikethrough(
                inlineContent: strikethrough.children.reduce(into: [], { result, child in result.append(contentsOf: visit(child)) }) as! [RenderInlineContent]
            )
        ]
    }
