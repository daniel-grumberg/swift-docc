    ...
    public convenience init?(from directive: BlockDirective, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        ...
        if let firstParagraph = directive.child(at: 0) as? Paragraph {
            ...
            let diagnostic = Diagnostic(
                ...
                identifier: "org.swift.docc.Resources.HasContent",
                summary: "\(Resources.directiveName.singleQuoted) directive requires a brief initial paragraph describing what the reader will find on the \(Resources.title) page"
            )
        ...
        Semantic.Analyses.HasOnlyKnownDirectives<Resources>(
            severityIfFound: .warning,
            allowedDirectives: Tile.DirectiveNames.allCases.map { $0.rawValue } + [Redirect.directiveName]
        )
            ...
            guard !seenTileDirectiveNames.contains(tile.title) else {
                    ...
                    let range = tile.originalMarkup.range
                {
                    let diagnostic = Diagnostic(
                        source: source,
                        severity: .warning,
                        range: tile.originalMarkup.range,
                        identifier: "org.swift.docc.Resources.DuplicateTile",
                        summary: "Duplicate child directive \(tileName.singleQuoted) in \(Resources.directiveName.singleQuoted)"
                    )
        ...
        for extraneousElement in remainder {
            let diagnostic = Diagnostic(
                source: source,
                severity: .warning,
                range: extraneousElement.range,
                identifier: "org.swift.docc.Resources.ExtraneousContent",
                summary: "Extraneous child element of \(Resources.directiveName.singleQuoted) directive"
            )
