...
public final class Tile: Semantic, DirectiveConvertible {
    ...
    fileprivate static func firstParagraph(
        of directive: BlockDirective,
        source: URL?,
        for bundle: DocumentationBundle,
        in context: DocumentationContext,
        problems: inout [Problem]
    ) -> (Paragraph?, remainder: MarkupContainer) {
        ...
        if let firstChild = directive.child(at: 0) {
            ...
            let diagnostic = Diagnostic(
                ...
                identifier: "org.swift.docc.Resources.\(directive.name).HasContent",
                summary: "\(directive.name.singleQuoted) directive requires an initial paragraph summarizing the contents of the tile's destination"
            )
    ...
    convenience init?(
        genericTile directive: BlockDirective,
        title: String,
        source: URL?,
        for bundle: DocumentationBundle,
        in context: DocumentationContext,
        problems: inout [Problem],
        shouldContainLinks: Bool = false
    ) {
        ...
        if shouldContainLinks, list == nil {
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.Resources.\(directive.name).HasLinks",
                summary: "\(directive.name.singleQuoted) directive should contain at least one list item"
            )
            problems.append(Problem(diagnostic: diagnostic, possibleSolutions: []))
        }
    ...
    public convenience init?(from directive: BlockDirective, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        ...
        default:
            ...
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.Resources.UnknownTile",
                summary: "Unknown \(directiveReference) of \(Resources.directiveName.singleQuoted); must be one of \(possibleTileDirectiveNames)"
            )
            problems.append(Problem(diagnostic: diagnostic, possibleSolutions: []))
            return nil
        }
