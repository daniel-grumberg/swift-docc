...
public final class Technology: Semantic, DirectiveConvertible, Abstracted, Redirected {
    ...
    public convenience init?(from directive: BlockDirective, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        ...
        let arguments = Semantic.Analyses.HasOnlyKnownArguments<Technology>(severityIfFound: .warning, allowedArguments: [Semantics.Name.argumentName])
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        Semantic.Analyses.HasOnlyKnownDirectives<Technology>(
            severityIfFound: .warning,
            allowedDirectives: [Intro.directiveName, Volume.directiveName, Chapter.directiveName, Resources.directiveName, Redirect.directiveName]
        )
        .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)
        ...
        let requiredIntro = Semantic.Analyses.HasExactlyOne<Technology, Intro>(severityIfNotFound: .warning)
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems).0

        var volumes: [Volume]
        var remainder: MarkupContainer
        (volumes, remainder) = Semantic.Analyses.HasAtLeastOne<Technology, Volume>(severityIfNotFound: nil)
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        // Retrieve chapters outside volumes.
        let chapters: [Chapter]
        (chapters, remainder) = Semantic.Analyses.HasAtLeastOne<Technology, Chapter>(severityIfNotFound: nil)
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)
                ...
                problems.append(
                    contentsOf:
                        chapters.map { chapter in
                            Problem(
                                diagnostic: Technology.isolatedChapterDiagnostic(isolatedChapter: chapter, source: source, range: chapter.originalMarkup.range),
                                possibleSolutions: []
                            )
                        }
                )
        ...
        (resources, remainder) = Semantic.Analyses.HasExactlyOne<Technology, Resources>(severityIfNotFound: nil)
    ...
    static func isolatedChapterDiagnostic(isolatedChapter: Chapter, source: URL?, range: SourceRange?) -> Diagnostic {
        return Diagnostic(
            ...
            summary:
                "Chapter should be in a \(Volume.directiveName.singleQuoted); either organize all Chapters in Volumes, or place them directly under your \(Technology.directiveName.singleQuoted)"
        )
    }
