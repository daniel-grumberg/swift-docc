...
private func invalidLinkDestinationProblem(destination: String, range: SourceRange?, severity: DiagnosticSeverity) -> Problem {
    let diagnostic = Diagnostic(
        ...
        summary: "Link destination \(destination.singleQuoted) is not a valid URL"
    )
...
private func disabledLinkDestinationProblem(reference: ResolvedTopicReference, range: SourceRange?, severity: DiagnosticSeverity) -> Problem {
    return Problem(
        diagnostic: Diagnostic(
            ...
            severity: severity,
            range: range,
            identifier: "org.swift.docc.disabledLinkDestination",
            summary: "The topic \(reference.path.singleQuoted) cannot be linked to."
        ),
        possibleSolutions: []
    )
}
...
private func unknownSnippetSliceProblem(snippetPath: String, slice: String, range: SourceRange?) -> Problem {
    let diagnostic = Diagnostic(
        ...
        severity: .warning,
        range: range,
        identifier: "org.swift.docc.unknownSnippetSlice",
        summary: "Snippet slice \(slice.singleQuoted) does not exist in snippet \(snippetPath.singleQuoted); this directive will be ignored"
    )
    return Problem(diagnostic: diagnostic, possibleSolutions: [])
}
...
private func removedLinkDestinationProblem(reference: ResolvedTopicReference, range: SourceRange?, severity: DiagnosticSeverity) -> Problem {
    ...
    let diagnostic = Diagnostic(
        ...
        severity: severity,
        range: range,
        identifier: "org.swift.docc.removedExtensionLinkDestination",
        summary: "The topic \(reference.path.singleQuoted) is an empty extension page and cannot be linked to.",
        explanation: "This extension symbol has had all its children curated and has been removed."
    )
...
struct MarkupReferenceResolver: MarkupRewriter {
    ...
    // precise diagnostics.
    var problemForUnresolvedReference:
        ((_ unresolvedReference: UnresolvedTopicReference, _ range: SourceRange?, _ fromSymbolLink: Bool, _ underlyingErrorMessage: String) -> Problem?)? = nil
