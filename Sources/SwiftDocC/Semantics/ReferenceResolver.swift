...
func unresolvedReferenceProblem(
    source: URL?,
    range: SourceRange?,
    severity: DiagnosticSeverity,
    uncuratedArticleMatch: URL?,
    errorInfo: TopicReferenceResolutionErrorInfo,
    fromSymbolLink: Bool
) -> Problem {
    var notes =
        uncuratedArticleMatch.map {
            [
                DiagnosticNote(
                    source: $0,
                    range: SourceLocation(line: 1, column: 1, source: $0)..<SourceLocation(line: 1, column: 1, source: $0),
                    message: "This article was found but is not available for linking because it's uncurated"
                )
            ]
        } ?? []
    ...
    let referenceSourceRange: SourceRange? = range.map { range in
        ...
        if fromSymbolLink {
            ...
            return SourceLocation(
                line: range.lowerBound.line,
                column: range.lowerBound.column + 5,
                source: range.lowerBound.source
    ...
    let diagnostic = Diagnostic(
        ...
        identifier: "org.swift.docc.unresolvedTopicReference",
        ...
        case let .failure(unresolved, error):
            ...
            problems.append(
                unresolvedReferenceProblem(
                    ...
                    range: range,
                    severity: severity,
                    uncuratedArticleMatch: uncuratedArticleMatch,
                    errorInfo: error,
    ...
    */
    ...
    mutating func visitTutorialSection(_ tutorialSection: TutorialSection) -> Semantic {
        ...
        return TutorialSection(
            originalMarkup: tutorialSection.originalMarkup,
            title: tutorialSection.title,
            introduction: newIntroduction,
            stepsContent: newStepsContent,
            redirects: tutorialSection.redirects
        )
    }
    ...
    mutating func visitTutorial(_ tutorial: Tutorial) -> Semantic {
        ...
        return Tutorial(
            originalMarkup: tutorial.originalMarkup,
            durationMinutes: tutorial.durationMinutes,
            projectFiles: tutorial.projectFiles,
            requirements: newRequirements,
            intro: newIntro,
            sections: newSections,
            assessments: newAssessments,
            callToActionImage: newCallToActionImage,
            redirects: tutorial.redirects
        )
    }
    ...
    mutating func visitTechnology(_ technology: Technology) -> Semantic {
        ...
        return Technology(
            originalMarkup: technology.originalMarkup,
            name: technology.name,
            intro: newIntro,
            volumes: newVolumes,
            resources: newResources,
            redirects: technology.redirects
        )
    }
    ...
    mutating func visitContentAndMedia(_ contentAndMedia: ContentAndMedia) -> Semantic {
        ...
        return ContentAndMedia(
            originalMarkup: contentAndMedia.originalMarkup,
            title: contentAndMedia.title,
            layout: contentAndMedia.layout,
            eyebrow: contentAndMedia.eyebrow,
            content: newContent,
            media: newMedia,
    ...
    mutating func visitChapter(_ chapter: Chapter) -> Semantic {
        ...
        let newTutorialReferencesWithoutDupes = newTutorialReferences.filter { newTutorialReference in
            guard !uniqueReferences.contains(newTutorialReference.topic) else {
                let diagnostic = Diagnostic(
                    source: chapter.originalMarkup.range?.source,
                    severity: .warning,
                    range: newTutorialReference.originalMarkup.range,
                    identifier: "org.swift.docc.\(Chapter.self).Duplicate\(TutorialReference.self)",
                    summary: "Duplicate \(TutorialReference.directiveName.singleQuoted) directive refers to \(newTutorialReference.topic.description.singleQuoted)"
                )
                let solutions =
        ...
        return Chapter(
            originalMarkup: chapter.originalMarkup,
            name: chapter.name,
            content: newContent,
            image: newImage,
            tutorialReferences: newTutorialReferencesWithoutDupes,
            redirects: chapter.redirects
        )
    }
    ...
    mutating func visitTutorialReference(_ tutorialReference: TutorialReference) -> Semantic {
        ...
        case .unresolved:
            let maybeResolved = resolve(
                tutorialReference.topic,
                in: bundle.technologyTutorialsRootReference,
    ...
    mutating func visitTutorialArticle(_ article: TutorialArticle) -> Semantic {
        ...
        return TutorialArticle(
            originalMarkup: article.originalMarkup,
            durationMinutes: article.durationMinutes,
            intro: newIntro,
            content: newContent,
            assessments: newAssessments,
            callToActionImage: newCallToActionImage,
            landmarks: article.landmarks,
