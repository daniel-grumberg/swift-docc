...
public final class MultipleChoice: Semantic, DirectiveConvertible {
    ...
    public convenience init?(from directive: BlockDirective, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        ...
        _ = Semantic.Analyses.HasOnlyKnownArguments<MultipleChoice>(severityIfFound: .warning, allowedArguments: [])
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        Semantic.Analyses.HasOnlyKnownDirectives<MultipleChoice>(severityIfFound: .warning, allowedDirectives: [Choice.directiveName, ImageMedia.directiveName])
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)
        ...
        if let paragraph = remainder.first as? Paragraph {
            ...
            let diagnostic = Diagnostic(
                ...
                identifier: "org.swift.docc.\(MultipleChoice.self).missingPhrasing",
                summary: " \(MultipleChoice.directiveName.singleQuoted) directive is missing its initial paragraph that serves as a question's title phrasing"
            )
        ...
        if choices.count < 2 || choices.count > 4 {
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.\(MultipleChoice.self).CorrectNumberOfChoices",
                summary: "`\(MultipleChoice.directiveName)` should contain 2-4 `\(Choice.directiveName)` child directives"
            )
            problems.append(Problem(diagnostic: diagnostic, possibleSolutions: []))
        }
        ...
        if correctAnswers.isEmpty {
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.\(MultipleChoice.self).CorrectChoiceProvided",
                summary: "`\(MultipleChoice.directiveName)` should contain `\(Choice.directiveName)` directive marked as the correct option"
            )
            problems.append(Problem(diagnostic: diagnostic, possibleSolutions: []))
            ...
            var diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.\(MultipleChoice.self).MultipleCorrectChoicesProvided",
                summary: "`\(MultipleChoice.directiveName)` should contain exactly one `\(Choice.directiveName)` directive marked as the correct option"
            )
        ...
        func removeExtraneous(_ elementName: String, range: SourceRange) -> Solution {
            return Solution(
                summary: "Remove extraneous code",
                replacements: [
        ...
        if codeBlocks.count > 1 {
            for extraneousCode in codeBlocks.suffix(from: 1) {
                ...
                let diagnostic = Diagnostic(
                    ...
                    severity: .warning,
                    range: directive.range,
                    identifier: "org.swift.docc.\(MultipleChoice.self).ExtraneousCode",
                    summary: "`\(MultipleChoice.directiveName)` may contain only one markup code block following the question's phrasing."
                )
        ...
        if images.count > 1 {
            for extraneousImage in images.suffix(from: 1) {
                ...
                let diagnostic = Diagnostic(
                    ...
                    severity: .warning,
                    range: directive.range,
                    identifier: "org.swift.docc.\(MultipleChoice.self).ExtraneousImage",
                    summary: "`\(MultipleChoice.directiveName)` may contain only one '\(ImageMedia.directiveName)' directive following the question's phrasing"
                )
        ...
        if codeBlocks.count == 1 && images.count == 1 {
            ...
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: directive.range,
                identifier: "org.swift.docc.\(MultipleChoice.self).CodeOrImage",
                summary: "`\(MultipleChoice.directiveName)` may contain an `\(ImageMedia.directiveName)` or markup code block following the question's phrasing"
            )
            let solutions = [
