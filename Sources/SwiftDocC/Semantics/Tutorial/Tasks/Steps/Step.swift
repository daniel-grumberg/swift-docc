...
public final class Step: Semantic, DirectiveConvertible {
    ...
    public convenience init?(from directive: BlockDirective, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        ...
        _ = Semantic.Analyses.HasOnlyKnownArguments<Step>(severityIfFound: .warning, allowedArguments: [])
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        Semantic.Analyses.HasOnlyKnownDirectives<Step>(severityIfFound: .warning, allowedDirectives: [ImageMedia.directiveName, VideoMedia.directiveName, Code.directiveName])
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        var remainder: MarkupContainer
        let optionalMedia: Media?
        (optionalMedia, remainder) = Semantic.Analyses.HasExactlyOneMedia<Step>(severityIfNotFound: nil)
            .analyze(directive, children: directive.children, source: source, for: bundle, in: context, problems: &problems)

        let optionalCode: Code?
        (optionalCode, remainder) = Semantic.Analyses.HasExactlyOne<Step, Code>(severityIfNotFound: nil)
            .analyze(directive, children: remainder, source: source, for: bundle, in: context, problems: &problems)

        let paragraphs: [Paragraph]
        (paragraphs, remainder) = Semantic.Analyses.ExtractAllMarkup<Paragraph>()
            .analyze(directive, children: remainder, source: source, for: bundle, in: context, problems: &problems)
        ...
        func diagnoseExtraneousContent(element: Markup) -> Problem {
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: element.range,
                identifier: "org.swift.docc.\(Step.self).ExtraneousContent",
                summary:
                    "Extraneous element: \(Step.directiveName.singleQuoted) directive should only have a single paragraph for its instructional content and an optional paragraph to serve as a caption"
            )
            let solutions =
                element.range.map {
        ...
        (blockQuotes, remainder) = Semantic.Analyses.ExtractAllMarkup<BlockQuote>()
            .analyze(directive, children: remainder, source: source, for: bundle, in: context, problems: &problems)

        for extraneousElement in remainder where (extraneousElement as? BlockDirective)?.name != Comment.directiveName {
            problems.append(diagnoseExtraneousContent(element: extraneousElement))
        }
        ...
        if content.isEmpty {
            let diagnostic = Diagnostic(
                ...
                identifier: "org.swift.docc.HasContent",
                summary: "\(Step.directiveName.singleQuoted) has no content; \(Step.directiveName.singleQuoted) directive should at least have an instructional sentence"
            )
        ...
        self.init(
            ...
            media: optionalMedia,
            code: optionalCode,
            content: MarkupContainer(content.elements),
            caption: MarkupContainer(caption.elements + blockQuotes as [Markup])
        )
    }
