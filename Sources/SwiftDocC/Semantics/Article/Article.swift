...
public final class Article: Semantic, MarkupConvertible, Abstracted, Redirected, AutomaticTaskGroupsProviding {
    ...
    convenience init(
        ...
        deprecationSummary: MarkupContainer?,
        metadata: Metadata?,
        redirects: [Redirect]?,
    ...
    public convenience init?(from markup: Markup, source: URL?, for bundle: DocumentationBundle, in context: DocumentationContext, problems: inout [Problem]) {
        guard let title = markup.child(at: 0) as? Heading, title.level == 1 else {
            ...
            let diagnostic = Diagnostic(
                ...
                severity: .warning,
                range: range,
                identifier: "org.swift.docc.Article.Title.NotFound",
                summary: "An article is expected to start with a top-level heading title"
            )
            ...
            problems.append(
                Problem(
                    diagnostic: Diagnostic(
                        source: source,
                        severity: .warning,
                        range: extraMetadata.originalMarkup.range,
                        identifier: "org.swift.docc.HasAtMostOne<\(Article.self), \(Metadata.self)>.DuplicateChildren",
        ...
        if !isDocumentationExtension, let metadata = optionalMetadata, let displayName = metadata.displayName {
            ...
            let diagnostic = Diagnostic(
                source: source,
                severity: .warning,
                range: metadata.originalMarkup.range,
                identifier: "org.swift.docc.Article.DisplayName.NotSupported",
