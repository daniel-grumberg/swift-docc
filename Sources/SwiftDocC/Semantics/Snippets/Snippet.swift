            ...
            problems.append(
                Problem(
                    diagnostic: Diagnostic(
                        ...
                        identifier: "org.swift.docc.EmptySnippetLink",
                        summary: "No path provided to snippet; use a symbol link path to a known snippet"
                    ),
                    possibleSolutions: []
                )
            )
...
extension Snippet: RenderableDirectiveConvertible {
    func render(with contentCompiler: inout RenderContentCompiler) -> [RenderContent] {
        ...
        guard let snippetReference = contentCompiler.resolveSymbolReference(destination: snippet.path),
            ...
            let snippetMixin = snippetSymbol.mixins[SymbolGraph.Symbol.Snippet.mixinKey] as? SymbolGraph.Symbol.Snippet
        else {
            return []
        }
