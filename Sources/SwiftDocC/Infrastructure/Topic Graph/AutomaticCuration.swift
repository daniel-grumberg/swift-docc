...
public struct AutomaticCuration {
    ...
    static var groups: ReferenceGroupIndex {
        return groupKindOrder.enumerated()
            .reduce(into: ReferenceGroupIndex()) { (result, next) in
    ...
    static func topics(
        ...
        let languagesFilter = Set(
            variantsTraits.compactMap {
                ...
                guard
                    inInheritedSymbolsAPICollection
                        || !(context.topicGraph.reverseEdges[reference] ?? [])
                            .contains(where: { GeneratedDocumentationTopics.isInheritedSymbolsAPICollectionNode($0, in: context.topicGraph) })
                else {
                    return
                }
    ...
    static func seeAlso(
        ...
        func filterReferences(_ references: [ResolvedTopicReference]) -> [ResolvedTopicReference] {
            references
                ...
                .filter { reference in
                    context.sourceLanguages(for: reference)
                        .contains(where: { language in
