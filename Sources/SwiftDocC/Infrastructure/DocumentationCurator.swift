...
struct DocumentationCurator {
    ...
    mutating func referenceFromLink(link: Link, resolved: ResolvedTopicReference, source: URL?) -> ResolvedTopicReference? {
            ...
            problems.append(
                Problem(
                    diagnostic: Diagnostic(
                        ...
                        severity: .warning,
                        range: link.range,
                        identifier: "org.swift.docc.InvalidDocumentationLink",
                        summary: "The link \((link.destination ?? "").singleQuoted) isn't valid",
                        explanation:
                            "Expected a well-formed URL that uses the \(ResolvedTopicReference.urlScheme.singleQuoted) scheme. You can only curate external links in a 'See Also' section."
                    ),
                    possibleSolutions: []
                )
            )
                ...
                problems.append(
                    Problem(
                        diagnostic: Diagnostic(
                            ...
                            severity: .warning,
                            range: link.range,
                            identifier: "org.swift.docc.SectionCuration",
                            summary: "The content section link \((link.destination ?? "").singleQuoted) isn't allowed in a Topics link group",
                            explanation: "Content sections cannot participate in the documentation hierarchy."
                        ),
                        possibleSolutions: []
                    )
                )
        ...
        guard let currentArticle = self.context.uncuratedArticles[reference],
            let documentationNode = try? DocumentationNode(reference: reference, article: currentArticle.value)
        else { return nil }
    ...
    mutating func crawlChildren(
        of nodeReference: ResolvedTopicReference,
        prepareForCuration: (ResolvedTopicReference) -> Void = { _ in },
        relateNodes: (ResolvedTopicReference, ResolvedTopicReference) -> Void
    ) throws {
        ...
        (taskGroups + authoredSeeAlsoGroups)
            .forEach({ group in
                problems.append(
                    contentsOf:
                        group.problemsForGroupLinks()
                        .map({ problem -> Problem in
        ...
        for (groupIndex, group) in taskGroups.enumerated() {
            for (linkIndex, link) in group.links.enumerated() {
                ...
                guard let childDocumentationNode = context.documentationCache[childReference],
                    childDocumentationNode.kind == .article || childDocumentationNode.kind.isSymbol || childDocumentationNode.kind == .tutorial
                        || childDocumentationNode.kind == .tutorialArticle
                else {
                    continue
                }
                ...
                var removeListItemSolutions: [Solution] {
                    ...
                    guard let listItem = sequence(first: link as (any Markup), next: \.parent).mapFirst(where: { $0 as? ListItem }),
                        ...
                        assertionFailure(
                            "Unable to find the list item element that contains \(link.format()) in the markup for \(describeForDiagnostic(nodeReference).singleQuoted)"
                        )
                        return []
                    }
                    return [
                        ...
                        problems.append(
                            Problem(
                                diagnostic: Diagnostic(
                                    ...
                                    severity: .warning,
                                    range: range(),
                                    identifier: "org.swift.docc.ModuleCuration",
                                    summary:
                                        "Organizing the module \(childReference.lastPathComponent.singleQuoted) under \(describeForDiagnostic(nodeReference).singleQuoted) isn't allowed",
                    ...
                    problems.append(
                        Problem(
                            diagnostic: Diagnostic(
                                ...
                                severity: .warning,
                                range: range(),
                                identifier: "org.swift.docc.CyclicReference",
