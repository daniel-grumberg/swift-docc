...
final class ExternalPathHierarchyResolver {
    ...
    func resolve(_ unresolvedReference: UnresolvedTopicReference, fromSymbolLink isCurrentlyResolvingSymbolLink: Bool) -> TopicReferenceResolutionResult {
        ...
        do {
            ...
            guard let foundReference = resolvedReferences[foundID] else {
                fatalError(
                    "Every identifier in the path hierarchy has a corresponding reference in the wrapping resolver. If it doesn't that's an indication that the file content that it was deserialized from was malformed."
                )
            }
            ...
            guard content[foundReference] != nil else {
                return .failure(
                    unresolvedReference,
                    .init("Resolved \(foundReference.url.withoutHostAndPortAndScheme().absoluteString.singleQuoted) but don't have any content to display for it.")
                )
            }
    ...
    func entity(_ reference: ResolvedTopicReference) -> LinkResolver.ExternalEntity {
        ...
        let topicReferences: [ResolvedTopicReference] = (resolvedInformation.references ?? [])
            .compactMap {
    ...
    init(
        ...
        self.pathHierarchy = PathHierarchy(fileRepresentation.pathHierarchy) { identifiers in
            ...
            for (index, path) in fileRepresentation.nonSymbolPaths {
                ...
                self.resolvedReferences[identifier] = ResolvedTopicReference(
                    bundleIdentifier: fileRepresentation.bundleID,
                    path: url.path,
