...
extension PathHierarchy.Error {
    ...
    func makeTopicReferenceResolutionErrorInfo(fullNameOfNode: (PathHierarchy.Node) -> String) -> TopicReferenceResolutionErrorInfo {
        ...
        switch self {
        case .moduleNotFound(let pathPrefix, let remaining, let availableChildren):
        ...
        case .notFound(let pathPrefix, let remaining, let availableChildren):
        ...
        case .nonSymbolMatchForSymbolLink(let path):
            return TopicReferenceResolutionErrorInfo(
        ...
        case .unknownDisambiguation(let partialResult, let remaining, let candidates):
            ...
            let solutions: [Solution] =
                ...
                .map { (node: PathHierarchy.Node, disambiguation: String) -> Solution in
                    return Solution(
                        summary: "\(Self.replacementOperationDescription(from: disambiguations, to: disambiguation)) for\n\(fullNameOfNode(node).singleQuoted)",
                        replacements: [
        ...
        case .unknownName(let partialResult, let remaining, let availableChildren):
            let nextPathComponent = remaining.first!
        ...
        case .lookupCollision(let partialResult, let remaining, let collisions):
            let nextPathComponent = remaining.first!
            ...
            let solutions: [Solution] = collisions.sorted(by: collisionIsBefore)
                .map { (node: PathHierarchy.Node, disambiguation: String) -> Solution in
                    return Solution(
