...
struct MergeAction: Action {
    ...
    private func validateThatArchivesHaveDisjointData() throws {
        ...
        for archive in archives {
            for typeOfDocumentation
                in (try? fileManager.contentsOfDirectory(at: archive.appendingPathComponent("data", isDirectory: true), includingPropertiesForKeys: nil, options: .skipsHiddenFiles))
                ?? []
            {
                for moduleOrTechnologyName in (try? fileManager.contentsOfDirectory(at: typeOfDocumentation, includingPropertiesForKeys: nil, options: .skipsHiddenFiles)) ?? [] {
                    archivesByTopLevelDirectory[typeOfDocumentation.lastPathComponent, default: [:]][moduleOrTechnologyName.deletingPathExtension().lastPathComponent, default: []]
                        .insert(archive.lastPathComponent)
                }
            }
        }
